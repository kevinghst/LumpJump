...
class Game{
  ...
  checkMonsterStep(){
    ...
    this.monsters.forEach( function(monster){
      ...
      && (that.poodle.y < monster.y + monster.height)){
        that.poodle.fallStop();
        that.poodle.jumpSpeed = 25;
        that.poodle.immune = true;
      }
    });
  }
    ...
    if(this.poodle.y > this.height*0.4){
      ...
      this.pads.forEach( function(pad){
        ...
        if(pad.y > that.height){
          ...
          let monsterRatio = .02 + (.18/10000) * that.points;
          if (monsterRatio > .2) { monsterRatio = .2 };
          let numThree = Math.random();
          if(numThree < monsterRatio) {
            that.monsters.unshift(new Monster(that.getRandomInt(0, that.width - 60), 300));
          }
        }
      });
